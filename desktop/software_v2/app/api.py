...
class ApiClient:
    ...
    # 软件代码：后台 DesktopDeviceController 按 app 字段归类，含「自动化/auto/web」→ auto。
    # 这样用户在后台按软件分别管理额度/付费，跟截图软件（pic）分开。
    APP_NAME = "好办法自动化"

    def register_device(self) -> dict:
        """走后台「桌面设备登记」接口（幂等：同一编号重复登记会复用已有账户）。
        后台据 app 字段把本软件归到 software_code=auto，并返回 token + quota。"""
        resp = safe_request(
            "POST",
            f"{API_BASE}/desktop/device/register",
            json={
                "software_id": self.serial,
                "app": self.APP_NAME,
            },
    ...
    def login_device(self) -> dict:
        """device 接口没有「密码登录」，凭已有 token 查状态即可。
        没有 token 时让 ensure_logged_in 走 register。"""
        if not self.token:
            raise RuntimeError("尚未登记设备")
        resp = safe_request(
            "GET",
            f"{API_BASE}/desktop/device/status",
            headers=self._headers(),
            timeout=15,
        )
        return resp.json()
    ...
    def ensure_logged_in(self) -> dict:
        # device register 幂等，直接登记一次即可拿到/复用账户与 token。
        # 失败兜底再试一次（防瞬时网络抖动）。
        try:
