...
() => {
    ...
    `;
    tip.textContent = '先登录、打开要操作的页面，再点上方按钮开始';
...
"""
    ...
    def execute(self, actions: list, row: Optional[dict] = None, loop_index: int = 1):
        ...
        while i < total:
            ...
            t = a.get("type", "")
            try:
                # 「点按钮 → 弹系统文件框 → 选图」组合（富文本 Quill 插图等）：录制会拆成
                # click + upload 两步，必须在点击同一刻用 expect_file_chooser 接住文件框。
                nxt = actions[i] if i < total else None
                if (t == "click" and nxt
                        and nxt.get("type") in ("upload", "upload_folder_to_library")
                        and not self._is_dialog_upload(nxt)):
                    chooser_files = self._resolve_upload_files(nxt, row)
                    if chooser_files:
                        handled = self._click_with_filechooser(a, chooser_files)
                        if handled:
                            i += 1  # 文件框已接管上传，跳过紧跟的 upload 步
                        self._manual_checkpoint_after_action(i, loop_index)
                        continue
                    # 上传列无值 → 当普通点击正常执行（落到下面 _run_one）
                self._run_one(a, row)
            except Exception as e:
    ...
    def _resolve_upload_files(self, a: dict, row: Optional[dict]) -> list:
        """从 upload 类 action 解析出要上传的文件列表（扫 from_excel 指定的文件夹）。
        无值 → 返回 []（调用方跳过）；路径错/空目录 → 抛 DSLError。
        """
        import os
        ...
        if not col:
            raise DSLError("上传步骤必须配置 from_excel（Excel 列名）")
        folder = ""
        used_col = col
        if row is not None:
            col_candidates = [col]
            ...
            for candidate in col_candidates:
                ...
                if value and value.lower() not in ("nan", "none"):
                    ...
                    break
        if not folder:
            self.log(f"⊝ Excel 列「{col}」无值，跳过上传")
            return []
        if not _P(folder).is_dir():
            raise DSLError(
                f"文件夹不存在：{folder}\n请检查 Excel 里「{used_col}」列的路径（要绝对路径）"
            )
        extensions = a.get("file_extensions", ["jpg", "jpeg", "png", "gif", "webp", "bmp"])
        ...
        if not files:
            raise DSLError(
                f"目录「{folder}」下没找到匹配图片\n允许的扩展名：{', '.join(sorted(extensions))}"
            )
        return files

    def _is_dialog_upload(self, upload_a: dict) -> bool:
        """upload 的目标 input 是否在素材库对话框/抽屉里。是 → 走原素材库逻辑
        （弹窗 + 勾选），不要被「点击+文件框」合并逻辑接管，以免破坏已正常的素材库上传。
        """
        sel = str(upload_a.get("selector", "")).lower()
        return "el-dialog" in sel or "el-drawer" in sel

    def _click_with_filechooser(self, click_a: dict, files: list) -> bool:
        """点 click_a 指向的按钮，并用 expect_file_chooser 接住它弹出的系统文件框，投入
        files。典型场景：富文本 Quill 插图 button.ql-image——录制会拆成 click + upload 两步，
        但点按钮弹出的文件框必须在点击同一刻接住，否则框被自动取消、上传失败。

        返回 True = 已接住文件框并投入文件（调用方跳过紧跟的 upload 步）；
        返回 False = 点击没弹出文件框（普通点击已执行），让 upload 步照常自己处理。
        """
        timeout = click_a.get("timeout", self.default_timeout)
        loc, _ = self._find_visible_locator(click_a, timeout)
        try:
            with self.page.expect_file_chooser(timeout=6000) as fc_info:
                loc.click(timeout=5000)
        except Exception as e:
            # 没弹出文件框 → 这是普通点击（已执行），交给后续 upload 步处理
            self.log(f"  （点击未弹出文件框，按普通点击处理：{str(e)[:60]}）")
            return False
        fc = fc_info.value
        to_set = files if fc.is_multiple() else files[:1]
        fc.set_files(to_set, timeout=20000)
        self.log(f"  ✓ 点按钮并选好 {len(to_set)} 个文件（文件框模式）")
        self.page.wait_for_timeout(max(800, len(to_set) * 200))
        return True
    ...
    def _run_one(self, a: dict, row: Optional[dict]):
        ...
        if t == "goto":
            ...
            # ────────────────────────────────────────────────
            files = self._resolve_upload_files(a, row)
            if not files:
                return
            n = len(files)
            ...
            # ⭐ 优先「点击上传按钮 → 拦截系统文件框」(file chooser 模式)。
            #    这是最通用的上传方式：富文本插图(Quill button.ql-image)、各种「上传」
            #    按钮、系统文件框都靠它；素材库点「上传」同样会弹文件框，传完后面再勾选。
            #    只有少数 SPA 把 input 直接塞在 DOM 里、点击不弹文件框，才回退 set_input_files。
            filechooser_done = False
            trigger_xpath = a.get("xpath") or ""
            if trigger_xpath:
                try:
                    trigger = self.page.locator(f"xpath={trigger_xpath}").first
                    with self.page.expect_file_chooser(timeout=6000) as fc_info:
                        trigger.click(timeout=4000)
                    fc = fc_info.value
                    # 非多选 input（如 Quill 富文本插图）一次只收一张，多给会报错
                    to_set = files if fc.is_multiple() else files[:1]
                    fc.set_files(to_set, timeout=20000)
                    filechooser_done = True
                    self.log(f"  ✓ 点「上传」按钮并选好 {len(to_set)} 个文件（文件框模式）")
                    self.page.wait_for_timeout(max(800, n * 200))
                except Exception as e:
                    self.log(f"  文件框模式未触发（{str(e)[:80]}），改用直接注入...")
            ...
            # ⭐ 没有素材库对话框/抽屉 = 直接插入式上传（如 Quill 富文本插图）：文件已上传/
            #    插入，没有勾选环节，立即返回——避免在没有 checkbox 的页面上空轮询十几秒像卡死。
            try:
                has_library = self.page.locator(
                    ".el-dialog__wrapper:visible, .el-drawer__wrapper:visible").count() > 0
            except Exception:
                has_library = True
            if not has_library:
                self.log("  （没有素材库对话框，文件已上传/插入，跳过勾选）")
                return
            ...
            if checkbox_loc is None:
                # 没有 checkbox 是正常情况：富文本插图(Quill)、直接插入式上传都没有素材库
                # 勾选环节——文件已通过文件框上传/插入，跳过勾选即可，绝不能报错中断。
                self.log("  （无素材库勾选环节，文件已上传，跳过勾选）")
                return
