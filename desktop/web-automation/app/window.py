...
class MainWindow(ctk.CTk):
    ...
    def _build_home(self):
        ...
        ctk.CTkLabel(el, text="浏览器", font=F_BODY, text_color=C_TEXT).pack(anchor="w")
        ...
        ctk.CTkButton(kb_local, text="查看明细",
        ...
        self.diag_btn = ctk.CTkButton(kb_btns, text="🔧 网络检测", width=110, height=34,
                                       font=F_BTN, corner_radius=7,
    ...
    def _build_record(self):
        ...
        ctk.CTkLabel(
            c1,
            text="💡 步骤多或要上传图片时，录制次数选 3 次更稳；按 Ctrl+Shift+X 可截图。",
            font=F_SM,
            text_color=C_ORANGE,
            wraplength=900,
            justify="left",
        ...
        ctk.CTkLabel(hr, text="操作步骤", font=F_BODY,
                     text_color=C_TEXT).pack(side="left")
        ...
        self.rec_status = ctk.CTkLabel(br, text="点「开始录制」开始，操作完点浏览器里的「完成」",
                                       font=F_SM, text_color=C_TEXT3)
    ...
    def _build_review(self):
        ...
        # 生成按钮（右）
        self.gen_btn = ctk.CTkButton(br, text="生成 Excel 模板",
        ...
        ctk.CTkLabel(br, text="AI 模型：", font=F_SM,
    ...
    def _show_review_empty(self):
        ...
        ctk.CTkLabel(empty, text="录制后会自动显示在这里",
    ...
    def _render_jst_workspace_body(self, tab):
        ...
        ctk.CTkLabel(
            intro,
            text="批量上架、改价、库存等操作：先选模块录一次，之后按 Excel 批量跑。",
            font=F_SM,
            text_color=C_TEXT2,
            wraplength=900,
            justify="left",
    ...
    def _build_jst_module_card(self, parent, mod: dict, row: int, col: int):
        ...
        if mod["ready"]:
            ...
            tk.Label(head, text=" 待设置 ", font=(FN, 9, "bold"),
    ...
    def _start_jst_module_recording(self, mod: dict):
        ...
        if not mod.get("url"):
            messagebox.showinfo("待设置", f"「{mod['label']}」还没设置入口网址，先点「🔍 设置入口网址」")
            return
            ...
            try:
                ...
                try:
                    self.after(0, lambda: self.jst_status.configure(
                        text="🚀 浏览器已打开，请打开目标页面...", text_color=C_GREEN))
    ...
    def _ask_save_module_url(self, mod: dict, default_url: str = ""):
        ...
        tk.Label(dlg, text="把浏览器地址栏的网址复制粘到下面（已自动填好最近打开的）：",
    ...
    def _jst_reset_login(self):
        ...
        try:
            ...
            messagebox.showinfo("已退出", "已退出登录，下次需要重新登录。")
        except Exception as e:
    ...
    def _start_jst_recording_with_url(self, target_url: str, module_key: str, module_label: str):
        ...
        try:
            ...
            self.rec_status.configure(
                text=f"🎬 「{module_label}」录制中，操作完点 ✅ 完成。",
                text_color=C_GREEN,
            )
        except Exception:
            pass
    ...
    def _open_jst_browser_only(self):
        ...
        def _launch():
            try:
                ...
                try:
                    self.after(0, lambda: self.jst_status.configure(
                        text="🚀 正在打开浏览器...", text_color=C_GREEN))
                    context, browser, edge_proc = launch_cdp_attached_context(
                        pw,
                        edge_profile_dir=JST_PROFILE_DIR,
                        initial_url=self.JST_TARGET_URL,
    ...
    def _run_jst_flow(self, flow_dir: Path):
        ...
        if not (flow_dir / "dsl.json").exists():
            messagebox.showerror("无法运行", f"找不到流程文件：{flow_dir.name}")
            return
    ...
    def _build_flows(self):
        ...
        self.flow_status = ctk.CTkLabel(status_row, text="", font=F_SM, text_color=C_TEXT3)
        self.flow_status.pack(side="left", anchor="w")
        # 运行时显示的「停止」按钮（默认隐藏，_run_flow 启动时 pack 出来）
        self.stop_btn = tk.Button(status_row, text="⏹ 停止", font=F_SM,
                                   relief="flat", bd=0, cursor="hand2",
                                   bg=C_RED, fg="#fff", activebackground="#b91c1c",
    ...
    def _run_flow(self, flow_dir: Path):
        ...
        if needs_excel:
            ...
            if answer:
                ...
                if not tpl.exists():
                    messagebox.showerror("找不到文件", "没找到「数据模板.xlsx」")
                    return
                excel_path = tpl
        ...
        self.flow_status.configure(
            text="浏览器打开后，先登录到目标页面，再点「开始工作」",
            text_color=C_BLUE)
        try:
            self.stop_btn.pack(side="right", padx=(8, 0), ipady=1)
        except Exception:
            pass

        def on_err(e):
            self.after(0, lambda: self._on_flow_finished(f"出错：{e}", C_RED))
            # 自动弹出错误反馈窗
        ...
        self.runner.run(
            ...
            on_done=lambda: self.after(0,
                lambda: self._on_flow_finished("✓ 运行完成！", C_GREEN)),
            on_error=on_err,
    ...
    def _stop_flow(self):
        """点「停止」：通知 runner 在当前这步/这行完成后优雅结束（不硬杀浏览器）。"""
        self.runner.stop()
        self.flow_status.configure(text="⏹ 正在停止...", text_color=C_RED)
        try:
            self.stop_btn.pack_forget()
        except Exception:
            pass

    def _on_flow_finished(self, text, color):
        """运行结束/出错统一收尾：更新状态文字 + 隐藏停止按钮。"""
        self.flow_status.configure(text=text, text_color=color)
        try:
            self.stop_btn.pack_forget()
        except Exception:
            pass
    ...
    def _build_pay_card(self, parent):
        ...
        if self._pay_qr_img is None:
            ctk.CTkLabel(qr_box, text="收款码加载中…\n如需充值请加客服微信",
        ...
        ctk.CTkLabel(rt, text="（充值、续费请加客服微信）",
...
class PatternsListDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, lib: PatternsLibrary):
        ...
        ctk.CTkLabel(head, text="🧠 AI 识别能力",
